...
<head>
    ...
    <title>Admit Animal - SafePaws</title>
...
<body class="text-white">
    <div class="max-w-2xl mx-auto px-5 py-8">
        <!-- Back Button -->
        <div class="mb-6">
            <a href="{{ route('rescue.reports.show', $report->id) }}" class="text-[#0ea5e9] hover:underline">← Back to Report</a>
        </div>

        <div class="bg-white/5 p-8 rounded-xl border border-white/10">
            <h1 class="text-3xl font-bold mb-2">Admit Animal from Report</h1>
            <p class="text-gray-400 mb-6">Create an animal record for Report #{{ $report->report_id }}</p>
            ...
            <form action="{{ route('rescue.animals.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="animal_report_id" value="{{ $report->id }}">

                <div class="mb-4">
                    <label class="block text-gray-300 mb-2">Name *</label>
                    <input type="text" name="name" value="{{ old('name') }}"
                           class="w-full bg-[#071331] border border-white/20 rounded-md px-4 py-2 text-white"
                           placeholder="Give a name to the rescued animal" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="species" class="w-full bg-[#071331] border border-white/20 rounded-md px-4 py-2 text-white" required>
                            ...
                            <option value="Dog" {{ (old('species') == 'Dog' || stripos($report->animal_type, 'dog') !== false) ? 'selected' : '' }}>Dog</option>
                            <option value="Cat" {{ (old('species') == 'Cat' || stripos($report->animal_type, 'cat') !== false) ? 'selected' : '' }}>Cat</option>
                            <option value="Rabbit" {{ (old('species') == 'Rabbit' || stripos($report->animal_type, 'rabbit') !== false) ? 'selected' : '' }}>Rabbit</option>
                            <option value="Bird" {{ (old('species') == 'Bird' || stripos($report->animal_type, 'bird') !== false) ? 'selected' : '' }}>Bird</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-gray-300 mb-2">Estimated Age (years) *</label>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3" class="w-full bg-[#071331] border border-white/20 rounded-md px-4 py-2 text-white">{{ old('description', $report->description) }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" class="w-full bg-[#071331] border border-white/20 rounded-md px-4 py-2 text-white" required>
                        <option value="Rescued" selected>Rescued</option>
                ...
                <div class="mb-6">
                    ...
                    @if($report->animal_photo)
                        <div class="mb-4">
                            <p class="text-sm text-gray-400 mb-2">Current Photo from Report:</p>
                            <img src="{{ asset($report->animal_photo) }}" class="h-48 rounded-lg object-cover border border-white/10" alt="Report Photo">
                            <p class="text-xs text-gray-500 mt-1">If you don't upload a new photo, this one will be used.</p>
                        </div>
                    @endif
                    ...
                    <div class="border-2 border-dashed border-white/20 rounded-md p-6 text-center">
                        ...
                        <label for="imageInput" class="cursor-pointer">
                            ...
                            <div class="text-gray-300">
                                <span class="text-[#0ea5e9] font-medium">Click to upload new</span> or drag and drop
                            </div>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('rescue.reports.show', $report->id) }}" class="outline-btn">Cancel</a>
                </div>
            </form>
        </div>
    </div>

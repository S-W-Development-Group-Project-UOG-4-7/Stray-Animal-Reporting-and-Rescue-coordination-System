...
<body class="text-white">
    ...
    <main class="max-w-7xl mx-auto px-5 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-6">
            ...
            <div class="filter-card">
                <div class="text-2xl font-bold text-orange-300">{{ $stats['homecheck'] }}</div>
                <div class="text-sm text-gray-300">Home Checks Needed</div>
            </div>
            <div class="filter-card">
                <div class="text-2xl font-bold text-green-300">{{ $stats['ready'] }}</div>
                <div class="text-sm text-gray-300">Ready for Approval</div>
            </div>
            <div class="filter-card">
                <div class="text-2xl font-bold text-blue-300">{{ $stats['completed_month'] }}</div>
            ...
            <div class="filter-card bg-gradient-to-r from-green-900/30 to-emerald-900/30">
                <div class="text-2xl font-bold text-green-300">{{ $stats['total_completed'] }}</div>
        ...
        <div class="card mb-6">
            ...
            <form method="GET" action="{{ route('rescue.adoptions') }}">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" class="filter-select" id="statusFilter">
                            ...
                            <option value="Home Check Scheduled" {{ request('status') == 'Home Check Scheduled' ? 'selected' : '' }}>Home Check Scheduled</option>
                            <option value="Approved" {{ request('status') == 'Approved' ? 'selected' : '' }}>Ready for Approval</option>
                    ...
                    <div>
                        ...
                        <select name="animal_type" class="filter-select" id="animalTypeFilter">
                            ...
                            <option value="Rabbit" {{ request('animal_type') == 'Rabbit' ? 'selected' : '' }}>Rabbit</option>
                    ...
                    <div>
                        ...
                        <select class="filter-select" id="priorityFilter" disabled>
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ request('date') }}" class="filter-input" id="dateFilter">
                    </div>
                    
                    <!-- Team Member Filter -->
                    <div>
                        <label class="filter-label">Assigned To</label>
                        <select name="assigned_to" class="filter-select" id="assignedFilter">
                ...
                <div class="flex gap-3 mt-4">
                    ...
                    <button type="button" class="outline-btn" onclick="window.location.href='{{ route('rescue.adoptions') }}'">Clear Filters</button>
                    <button type="button" class="success-btn" onclick="exportAdoptionData()">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        Export Data
                    </button>
                    <button type="button" class="warning-btn" onclick="scheduleHomeChecks()">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="card">
            <form id="batchApproveForm" action="{{ route('rescue.adoptions.batchApprove') }}" method="POST">
                @csrf
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-bold">Adoption Applications</h2>
                    <div class="flex gap-2">
                        <a href="{{ route('rescue.adoptions.create') }}" class="primary-btn text-sm inline-flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            Create Record
                        </a>
                        <button type="button" class="outline-btn text-sm" onclick="window.location.reload()">Refresh</button>
                        <button type="button" class="approve-btn text-sm" onclick="approveSelected()">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            Approve Selected
                ...
                <div class="table-container">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="adoption_ids[]" value="{{ $adoption->id }}" class="rounded adoption-checkbox"></td>
                            <td class="font-medium">#ADP-{{ $adoption->id }}</td>
                            <td>
                                <div class="font-medium">{{ $adoption->full_name }}</div>
                                <div class="text-xs text-gray-400">{{ $adoption->housing_type }}@if($adoption->reason) • {{ Str::limit($adoption->reason, 30) }}@endif</div>
                            </td>
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        <div class="font-medium">{{ $adoption->animal->name }} ({{ $adoption->animal->species }})</div>
                                        <div class="text-xs text-gray-400">#SP{{ str_pad($adoption->animal->id, 3, '0', STR_PAD_LEFT) }} • {{ $adoption->animal->breed }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="status-badge 
                                    @if($adoption->status == 'Pending' || $adoption->status == 'Pending Review') status-pending
                                    @elseif($adoption->status == 'Home Check Scheduled') status-homecheck
                                    @elseif($adoption->status == 'Ready for Pickup' || $adoption->status == 'Approved') status-approved
                            ...
                            <td>
                                <div class="text-white">{{ $adoption->created_at->format('M d, Y') }}</div>
                                <div class="text-xs text-gray-400">{{ $adoption->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                @if($adoption->assignedUser)
                                    <div class="flex items-center gap-2">
                                        <div class="w-6 h-6 bg-[#0ea5e9] rounded-full flex items-center justify-center text-xs">
                                            {{ strtoupper(substr($adoption->assignedUser->name, 0, 1)) }}
                                        </div>
                                        <span>{{ $adoption->assignedUser->name }}</span>
                                    </div>
                                @else
                            ...
                            <td>
                                <div class="flex gap-2 flex-wrap">
                                    <a href="{{ route('rescue.adoptions.show', $adoption->id) }}" class="text-xs outline-btn px-3 py-1">View</a>

                                    @if($adoption->status == 'Pending' || $adoption->status == 'Pending Review')
                                        <form action="{{ route('rescue.adoptions.approve', $adoption->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs success-btn px-3 py-1">Approve</button>
                                        </form>
                                        <form action="{{ route('rescue.adoptions.reject', $adoption->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs danger-btn px-3 py-1">Reject</button>
                                        </form>
                                    @elseif($adoption->status == 'Home Check Scheduled')
                                        <form action="{{ route('rescue.adoptions.approve', $adoption->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs success-btn px-3 py-1">Approve</button>
                                        </form>
                                    @elseif($adoption->status == 'Ready for Pickup' || $adoption->status == 'Approved')
                                        <form action="{{ route('rescue.adoptions.approve', $adoption->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs success-btn px-3 py-1">Complete</button>
                                        </form>
                                    @elseif($adoption->status == 'Completed')
                                        <span class="text-xs text-green-400">✓ Adopted</span>
                                    @else
    ...
    <script>
        ...
        function approveSelected() {
            const checkboxes = document.querySelectorAll('.adoption-checkbox:checked');

            if (checkboxes.length === 0) {
                alert("Please select at least one adoption application to approve.");
                return;
            }

            const confirmed = confirm(`Approving selected adoption applications...\n\nSelected applications will be:\n- Moved to 'Ready for Pickup'\n- Applicants notified via email\n- Animals marked as 'Adopted'\n- Records updated in system\n\nDo you want to continue?`);
